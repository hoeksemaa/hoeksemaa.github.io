...
<div class="essay-buttons">
  <a class="essay-button" href="https://hoeksemaa.github.io/brain-pong/"><span><i class="ph-bold ph-database"></i>public data portal</span></a>
  <a class="essay-button" href="https://github.com/hoeksemaa/brain-pong"><span><i class="ph-bold ph-github-logo"></i>project github</span></a>
  <a class="essay-button" href="https://x.com/trillionjohns/status/2078159497898381385"><span><i class="ph-bold ph-x-logo"></i>tournament twitter thread</span></a>
</div>
...
My first [BCI](https://en.wikipedia.org/wiki/Brain%E2%80%93computer_interface) was properly medieval. 6 saltwater sponges were held together with a plastic headband.
...
I mixed the saltwater by hand, adding generous amounts of sea salt to a bowl of warm water. The whole headband was dunked into the bowl to wet the sponges. The sponges sat near the back of the head to give them good contact with the occipital lobe. They were rudimentary [EEG](https://en.wikipedia.org/wiki/Electroencephalography) electrodes. However, this did little to mitigate the feeling of water dripping down your back. Only brave souls not afraid of a little water would wear it.

The headband still needed to be plugged into a Cerelog BCI board.
...
The board converts signals from analog to digital and gives my computer access to those values. Once the values were connected to the Pong script, the game could be played.

I inherited the device from a previous intern while interning at Cerelog, a BCI board design company. The intern had built the headband, and wrote software using [SSVEP](https://en.wikipedia.org/wiki/Steady_state_visually_evoked_potential) that analyzed the signals coming from the sponge electrodes. Flashing lights were on either side of the pong screen; the left flashed at 15 Hz and the right flashed at 10 Hz. How was looking at flashing lights converted into controls for Pong?

<div class="blue-box" markdown="1">
<span class="blue-box-title">SSVEP Explainer</span>

<span class="img-link"><img src="/assets/images/brain-pong-ssvep-fft.jpg" alt="A power spectrum plot with clear peaks at 15 Hz, 30 Hz, and 45 Hz."><a class="img-link-badge" href="https://onlinelibrary.wiley.com/doi/10.1155/2010/702357" target="_blank" rel="noopener noreferrer" aria-label="Open the source paper for this image"><i class="ph-bold ph-link"></i></a></span>

Looking at the graph above, I would claim that the subject is currently looking at a 15 Hz flashing light, not 10 Hz. When the eyes are stimulated by a light flashing at a certain frequency, the neurons of the occipital lobe (the portion of the brain processing sight) pulse at that same frequency. The neurons’ signal can be measured with electrodes. When you examine the frequencies of the electrode signal [via FFT](https://en.wikipedia.org/wiki/Fast_Fourier_transform), there is a clear spike at the frequency of the flashing lights (in this case 15 Hz). There are also harmonic effects, which is why you see spikes at 30 Hz and 45 Hz.
</div>
...
I felt I could build a more reliable BCI, and set out looking for other options.

Simplicity was important to me. I wanted to prove I could build a BCI at all, without worrying about the usefulness of the device yet. Through research, [EOG](https://en.wikipedia.org/wiki/Electrooculography) emerged as the clear frontrunner.

<div class="blue-box" markdown="1">
<span class="blue-box-title">EOG Explainer</span>

<span class="img-link"><img src="/assets/images/brain-pong-eog-electrode-placement.jpg" alt="A diagram of a head showing electrode placement: right hEOG and left hEOG beside the eyes, reference behind the ear."><a class="img-link-badge" href="https://www.nature.com/articles/s41598-018-27865-5" target="_blank" rel="noopener noreferrer" aria-label="Open the source paper for this image"><i class="ph-bold ph-link"></i></a></span>
...
The signals from EOG are clear, large enough, and correspond to known actions. In contrast, EEG signals don’t have a clear 1:1 mapping between actions and signals.

I also got my hands on a set of gold electrodes.
...
Gold cup electrodes serve the same purpose as saltwater sponge electrodes: conduct signal. They have several advantages. Their [impedance](https://en.wikipedia.org/wiki/Electrical_impedance) (similar to electrical resistance) is low when combined with proper skin preparation and [Ten20 conductive gel](https://www.weaverandcompany.com/products/ten20/). The gold plating also resists corrosion, meaning they last longer and can be regularly washed and reused. In absolute terms, gold has worse impedance than silver/silver chloride electrodes (and perhaps even the sponge electrodes? didn’t test). However, the gold electrodes have worked well for me so far.
</div>

Honestly, the reason I bought these is because [OpenBCI recommends them on their site](https://docs.openbci.com/GettingStarted/Biosensing-Setups/EEGSetup/). I’d seen successful products using an OpenBCI board in the past and had confidence the electrodes would work well enough for my purposes.
...
What signal does it send? There’s still noise in the left eye and right eye electrodes. The ADS1299 calculates the average signal between the two electrodes and pumps it back into the body via the bias electrode. This signal is in the mV range and shouldn’t hurt anyone. Most of this noise is common mode noise. The bias helps drive the body’s common mode noise close to zero volts so that the measured signals are within the chip’s measurable rails.
...
The real-world analog values are now converted into digital ones. This is done using an [ADC](https://en.wikipedia.org/wiki/Analog-to-digital_converter).

<div class="blue-box" markdown="1">
<span class="blue-box-title">ADC Explainer</span>
...
My Cerelog board samples at 250 Hz (it can sample faster but the data output becomes a problem). However, in this animation, we’re sampling at 10 Hz because it’s easier to see. The analog data is converted via the board to a 24 bit value.

<div id="fig-05" class="eog-fig"></div>

All of this so far has been done on **hardware**, primarily the ADS1299 chip. Once digitization is finished, the values are sent off to the computer to be processed by software. All future steps are done in **software**.

### 5. subtract left from right {#step-5}

We now have 2 raw data channels: the left and right electrodes. We don’t particularly care about the exact reading of right or left, but the **difference** between them.
...
The left and right electrode voltages have a tendency to drift over time. This is due to gel drying, the exact contact shifting, and some electrical effects. These factors are usually slightly different across electrodes, leading to them drifting different amounts. Even after the two channels are subtracted from each other, there can still be a noticeable slope over time, shown in the image below.

Detrend takes the gradual, zoomed-out slope of a graph and flattens it to zero. It also has the added benefit of setting the y-axis value of the signal to zero, which can be helpful for filtering.

<figure class="essay-figure">
  <span class="img-link"><img src="/assets/images/brain-pong-detrend-drift.png" alt="An EOG trace after DC offset removal, with a green arrow tracing a steady upward drift across four seconds."><a class="img-link-badge" href="https://www.researchgate.net/figure/Optimization-of-signal-processing-and-feature-extraction-for-real-time-data_fig3_368375242" target="_blank" rel="noopener noreferrer" aria-label="Open the source for this image"><i class="ph-bold ph-link"></i></a></span>
  <figcaption>over time, the signal trends up. If you’d like to see someone else’s EOG processing pipeline, click the image’s link.</figcaption>
</figure>

<div id="fig-07" class="eog-fig"></div>

My detrend is very aggressive. It flattens the signal more than I’d want. I’d like to adjust it to take effect over a period of 10 seconds instead of the ~0.5 seconds it takes to bring the signal’s value to zero.
...
Finally, we’ve made it to what I would judge to be a somewhat “clean” signal. Notice all of those spikes that have popped out? Those indicate clear left and right motions of the eyes.

How do we judge what counts as a spike? Spikes can vary by height. A simple way is to plop the subject in the chair, wire them up to the electrodes, turn on the software, and record them for several seconds. It’s good to ask them not to move their head, blink, swallow, or look around. This establishes a clean electrical “baseline”, aka a “noise floor”. We can derive a sigma value that defines a “noisyness” for their data.

<div id="fig-09" class="eog-fig"></div>

I use the following [median absolute deviation](https://en.wikipedia.org/wiki/Median_absolute_deviation) formula to calculate the exact sigma value:

<p class="essay-equation">σ = 1.4826 × MAD</p>

### 9. detect spikes {#step-9}

We use the sigma value multiplied by some constant to establish lines above and below the zero line. If the graph crosses it, congrats! Spike detected.

<div id="fig-10" class="eog-fig"></div>

This is the entire output of the processing pipeline: a series of positive and negative spikes (both red but recorded differently) at various timestamps. We can now use these spikes to drive the Pong paddle.

The control scheme is a little wonky: when a left spike is followed by a right spike in a certain window, the paddle moves left (vice versa with right followed by left). This is done because live detection of eye angle seemed tricky to detect, and large left/right eye movements seemed easy. However, if you want to move your paddle left 3 times, looking further left 3 times might rotate your eyes out of your skull. Thus, the “glance pair” was born: **look left and then back to center to move the paddle left.**
...
<figure class="essay-figure">
  ...
  <figcaption>the first working EOG demo! I’m controlling the green paddle on the bottom</figcaption>
</figure>

Fair warning: The hardware noise in these animations is a guess. The software signals from the “subtract left from right” animation onwards are real recordings from my own R/L electrodes. I took a guess at reasonable environmental noise and extrapolated backwards for animations 1-4. The SRB1 and Bias data are spoofed to common frequencies and amplitudes; the real ADC sampling is significantly faster (250 Hz). All the math, however, is real.

## path of pain {#path-of-pain}

After the initial success of the pipeline, I got cocky. The pipeline still had bugs but I wanted to optimize.

The SRB1 pin, as mentioned earlier, provides an electrical baseline. Measurements against it provide the electrical baseline your whole body is at so that this value can be subtracted out from individual left and right electrodes. SRB1 is subtracted from both left and right. That means that when I subtract left from right to create a single wave, the subtraction of SRB1 from both should cancel out. It seemed like an unnecessary part, so I thought deleting it would simplify the system. In practice, it led to more pain.
...
SRB1 is the ground from which the voltage is measured from. If the signal measures the top of a building, SRB1 measures the ground floor. When the SRB1 is removed, there’s no ground floor to measure against, so the height of the building is impossible to know. Voltage is measured relative to other voltages, not absolutely.

The second reason is actually unrelated to SRB1. When changing the firmware to ignore SRB1 (aka differential input mode), I accidentally turned on all 8 channels as input to the bias signal (via firmware). Previously, the bias was only calculated from the left and right electrode channels. This means that when the bias was calculating which signal average to generate, it used 2 channels of real signal and 6 channels of garbage unconnected electrical noise (nothing was connected to channels 3-8).
...
Even after I turned SRB1 back on, the signal still didn’t improve. I went a little crazy considering a whole host of theories for why my data was garbage. My raw data was noisy, so most of them related to connection quality, like:
...
After a while of pulling out my hair, I took another look at the firmware and discovered the 8-channel bias error. It was a simple fix to drop bias back to 2 channels and reflash the board.
...
Ideally, all electrodes will have both a **low** and a **similar** impedance. If one electrode has a different impedance, the chip cannot fully reject [common-mode interference](https://en.wikipedia.org/wiki/Common-mode_signal) (especially from 60 Hz mains).
...
These changes largely solved their associated noise issues.
...
Useful software changes precipitated out of the new data. The largest glaring problem was that my amplitude detector was failing to capture a majority of the prompted glances in my new dataset. It had a 45% failure rate using a 6σ detection band when tested on the 10 new people. Amplitude filtering worked well on me, but when tested on others, it could fail spectacularly. This was obviously unworkable.
...
As in the “detect spikes!” animation, all signals above and below the lines are recorded as spikes. These spikes are converted into paddle movements.
...
<figure class="essay-figure">
  ...
  <figcaption>a subject kindly giving me his eyeball data</figcaption>
</figure>

## time for a tournament {#time-for-a-tournament}

To finally put the technology to the test, I hosted a [Brain Pong tournament](https://luma.com/74gvtjzb). Everything was in place: the competitors had been signed up, the eyeball-shaped snacks had been purchased, and the electrodes had been labelled and dried. All that’s left is to see how the tech performed.
...
There were some steps I took to mitigate tournament noise. Foolishly I tried again to see if an HDMI projector would work, as the center room had a nicer screen for tournaments. This failed and I switched to the sideroom wireless projector and used Screen Mirroring. This successfully reduced noise.
...
The summer heat was bearing down on the contestants as they arrived at the tournament. Many of them showed up sweaty. I had a suspicion this was creating unnecessary noise recorded by the electrodes; asking them to wash their face with soap seemed to help.

Some contestants had fantastically low noise, while others basically couldn’t get a clear signal at all. Sometimes I feel like I’m taking crazy pills; that electrons hate me personally; that some people are secretly robots that have 10x the electrical noise from real humans. The process of getting better [SNR](https://en.wikipedia.org/wiki/Signal-to-noise_ratio) across 10+ people has felt a bit like a knife fight in the dark, where only excessive paranoia and problem isolation allows me to make progress. Any way I can lower the odds of a problem occurring is a gift.
...
- the founder of [Cerelog](https://www.cerelog.com/), the company designing the BCI boards I use
...
The tournament was chaotic. The technology worked smoothly on probably less than 50% of people. It’s frustrating when the controller doesn’t respond as you expect it to. When I throw the next tournament, game control accuracy is my biggest focus. I want people to be able to feel like they’re in control.
...
So long; I hope you liked the eyeball-shaped snacks!

# part 2: reflections {#part-2-reflections}

## how did I screw up? {#how-did-i-screw-up}

How could I have conducted this process better, from prototype to tournament? It’s traditional to conduct a post-mortem after software incidents ([Crowdstrike 2024](https://www.crowdstrike.com/wp-content/uploads/2024/08/Channel-File-291-Incident-Root-Cause-Analysis-08.06.2024.pdf?_sp=e2d7d9c2-bf48-4315-a14c-c9d53690af1b)), and the military conducts [hot washes](https://en.wikipedia.org/wiki/Hotwash) after missions. I want to take some time to reflect on how to do better.
...
**My greatest sin was not [sprinting to a full integrated test](https://grantobi.substack.com/i/177143333/sprint-to-your-first-all-up-integrated-test).** I had a working one-player version in May! When one-player was working, I should have immediately modified the game to support two-player and tested it until it broke. Integrated tests usually reveal major problems, and I should have tried to find these problems as soon as possible.
...
On my next project, I’ll specify a series of progressively harder tests to run. They’ll start with very simple tests (does the board turn on?) and culminate in the full integrated test. Then, I can ruthlessly knock out successful tests and keep my eye on the final test.
...
<figure class="essay-figure">
  ...
  <figcaption>what am I trying to solve for, really?</figcaption>
</figure>
...
On my next project, I’ll make a point to explicitly do each step and document my results at each step. This will force me to not only do the work, but come to a legible decision at each step, so I can’t throw my hands up and say “ok, that’s probably enough brainstorming. on to the prototype!”
...
I wrote basically 100% of this project with AI tools. Like many other developers, I haven’t written a line of code by hand for a while. I should have given the agent environment more attention; this would have been a major force multiplier for coding progress. This mostly involves leaning into the [Bitter Lesson](http://www.incompleteideas.net/IncIdeas/BitterLesson.html): general methods outperform structured-but-limiting ones in the long run. Give your agents more context about you, your project, and your desired workflow. Remove requests for your agents to work in a certain way unless it consistently gets things wrong (but check again in another 6 months). More context; fewer limitations. Follow [@\_\_drewface](https://x.com/__drewface) or [@\_\_\_Atin\_\_\_](https://x.com/___Atin___) on twitter if you want to supercharge your agent workflow! While I did use LLMs to code this project, I’m not convinced that the time of hand-written software for safety-critical systems like BCIs is over. However, the industry seems to be moving towards AI systems that supercharge testing and observability of code instead of more human effort.
...
- ADC: [analog-digital converter](https://en.wikipedia.org/wiki/Analog-to-digital_converter)
- ADHD: [Attention-Deficit/Hyperactivity Disorder](https://www.nimh.nih.gov/health/topics/attention-deficit-hyperactivity-disorder-adhd)
- AI: artificial intelligence
- BCI: [brain-computer interface](https://en.wikipedia.org/wiki/Brain%E2%80%93computer_interface)
...
- SNR: [signal-noise ratio](https://en.wikipedia.org/wiki/Signal-to-noise_ratio)
